...
def naiveSoftmaxLossAndGradient(
    ...
    d_y = y_hat.copy()
    d_y[outsideWordIdx] -= 1
    gradCenterVec = np.dot(outsideVectors.T, d_y)
    gradOutsideVecs = np.outer(d_y, centerWordVec)
...
def negSamplingLossAndGradient(
    ...
    factor_1 = np.dot(outsideVectors[outsideWordIdx].T, centerWordVec)
    neg_samples = outsideVectors[negSampleWordIndices]
    factor_2 = np.dot(-neg_samples, centerWordVec)
    loss = -np.log(sigmoid(factor_1)) - np.sum(np.log(sigmoid(factor_2)))

    gradCenterVec = -(1 - sigmoid(factor_1)) * outsideVectors[outsideWordIdx] + np.sum((np.expand_dims(1 - sigmoid(factor_2), axis=1) * neg_samples), axis=0)

    gradOutsideVecs = np.zeros(shape = outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] = -(1 - sigmoid(factor_1)) * centerWordVec
    for i, neg_idx in enumerate(negSampleWordIndices):
        gradOutsideVecs[neg_idx] += (1-sigmoid(factor_2)[i]) * centerWordVec
